...
export default function ConstructorMonster({ onError, saveImage, spriteSheets,  spriteAtlas}: ConstructorMonsterProps) {
  ...
  useEffect(() => {
    ...
    for (const frameName in spriteAtlas.frames) {
      if (frameName.includes('/stay/')) {
        ...
        } else if ((category.startsWith('right_arm') || category.startsWith('left_arm')) && frameName.endsWith('_0')) {
  ...
  const moveSelectedPart = (direction: 'left' | 'right') => {
    ...
    const newIndex = direction === 'left' ? (count - 1 + parts.length) % parts.length : (count + 1) % parts.length;
  ...
  const handleSelectedPart = (index: number) => {
    ...
    if (typePart === 'arms' && 'arm' in selectedFrame) {
      ...
      selectedPartsMonster.current.leftArm = selectedArmPair.left ? extractPartInfo(selectedArmPair.left) : undefined;
      selectedPartsMonster.current.rightArm = selectedArmPair.right ? extractPartInfo(selectedArmPair.right) : undefined;
  ...
  useEffect(() => {
    ...
    function updateDisplay(scene: Phaser.Scene) {
      ...
      if (selectedPartsMonster.current.rightArm) {
        drawPart(selectedPartsMonster.current.rightArm, bodyPoints?.attachRightArm || { x: 0, y: 0 });
      }
    ...
    <div>
      ...
      <div>
        <div className={styles.wrapperCreateMenu}>
          <button className={styles.buttonLeftRight} onClick={() => moveSelectedPart('left')}>◀</button>
          <PartCarousel partPreviews={partPreviews} typePart={typePart} count={count} handleSelectedPart={handleSelectedPart} spriteSheets={spriteSheets} />
          <button className={styles.buttonLeftRight} onClick={() => moveSelectedPart('right')}>▶</button>
        </div>
        <div className={styles.wrapperCreateMenu}>
        ...
        <div className={styles.wrapperCreateMenu}>
          <input type="text" placeholder="Введите имя" value={monsterName} onChange={(e) => setMonsterName(e.target.value)} className={styles.nameInput} />
